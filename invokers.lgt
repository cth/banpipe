...
:- object(prolog_invoker, implements(invokerp)).
	...
	run(InterfaceFile,Goal) :-
		...
		atomic_list_concat([Exec,' ', GoalOption, ' "assertz(task(_)), assertz(invoke_with(_)), logtalk_load(banpipe(fix_directives)), logtalk_load(\'', BaseFileName, '\',[hook(fix_directives)]),', GoalAtom,',halt."'],Command),
		write(shell_command(Command)), nl,
		os::shell(Command),
		os::change_directory(CurrentDir).
